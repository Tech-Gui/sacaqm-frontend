    ...
    try {
      ...
      const [hR] = await Promise.all([
        axios.get(`${BASE}/api/nodedata/aggregated`, {
          params: { sensor_id: sid, start: fStart, end: fEnd, resolution: 'hourly' },
          headers: token ? { Authorization: `Bearer ${token}` } : {}
        }).catch(() => ({ data: [] })),
      ]);
          ...
          <Grid container spacing={3} alignItems="center">
            ...
            <Grid item xs={12} md={4}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
                <Button variant="outlined" onClick={() => navigate("/private-summary")} startIcon={<span>⬅️</span>} sx={{ textTransform: 'none', borderRadius: 3, border: '2px solid #3b82f6', color: '#3b82f6', fontWeight: 600, '&:hover': { bgcolor: 'rgba(59,130,246,0.08)', borderColor: '#2563eb' } }}>
                  Back to Summary
                </Button>
