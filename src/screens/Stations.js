...
import { Button, Card, Col, Container, Form, Modal, Table } from "react-bootstrap";
...
const Stations = () => {
  ...
  const { stations, loading, error, fetchStations } = useContext(StationContext);
  ...
  const handleSubmit = async () => {
    try {
      await axios.post("https://try-again-test-isaiah.app.cern.ch/api/stations", formData);
      handleCloseModal();
    } catch (error) {
  ...
  return (
    <div className="d-flex flex-row" style={{ minHeight: "100vh", maxHeight: "100vh", background: "#f2f2f2" }}>
      <Sidebar />
      <Container fluid className="p-4">
        <TopNavBar />
        <div className="mb-4 d-flex flex-row justify-content-between">
          <h4 style={{ fontSize: "23px", fontWeight: "500", textAlign: "left" }}>Stations</h4>
        ...
        <Modal.Body>
          <Form>
            ...
            <Form.Group controlId="formLongitude">
              ...
              <Form.Control type="text" name="longitude" value={formData.longitude} onChange={handleChange} />
            </Form.Group>
            <Form.Group controlId="formLatitude">
              <Form.Label>Latitude</Form.Label>
              <Form.Control type="text" name="latitude" value={formData.latitude} onChange={handleChange} />
            </Form.Group>
            ...
            {formData.sensorIds.map((sensorId, index) => (
              <Form.Group controlId={`formSensorID${index}`} key={index}>
                ...
                {formData.sensorIds.length > 1 && (
                  <Button variant="danger" onClick={() => removeSensorId(index)} className="mt-2">
                    Remove
                  </Button>
                )}
              </Form.Group>
