...
// const THRESHOLDS = { pm1: 50, pm25: 60, pm5: 75, pm10: 100, noise: 70, temperature: 32, humidity: 80, co2: 1000, nox: 200, voc: 500 };
...
//           { parameter: "PM1.0", status: statuses.pm1,   exceedances: curr.filter((d) => (d.pm1p0       || 0) > THRESHOLDS.pm1).length },
//           { parameter: "PM2.5", status: statuses.pm25,  exceedances: curr.filter((d) => (d.pm2p5       || 0) > THRESHOLDS.pm25).length },
//           { parameter: "PM4.0", status: statuses.pm5,   exceedances: curr.filter((d) => (d.pm4p0       || 0) > THRESHOLDS.pm5).length },
//           { parameter: "PM10",  status: statuses.pm10,  exceedances: curr.filter((d) => (d.pm10p0      || 0) > THRESHOLDS.pm10).length },
//           { parameter: "Noise", status: statuses.noise, exceedances: curr.filter((d) => (d.dba         || 0) > THRESHOLDS.noise).length },
//           { parameter: "Humidity", status: statuses.humidity, exceedances: curr.filter((d) => (d.humidity || 0) > THRESHOLDS.humidity).length },
//           // Only add CO2 if sensor has data
//           ...(curr.some((d) => (d.co2 || 0) > 0) ? [{ parameter: "CO2", status: statuses.co2, exceedances: curr.filter((d) => (d.co2 || 0) > THRESHOLDS.co2).length }] : []),
//           { parameter: "NOx",   status: statuses.nox,   exceedances: curr.filter((d) => (d.nox         || 0) > THRESHOLDS.nox).length },
//           { parameter: "VOC",   status: statuses.voc,   exceedances: curr.filter((d) => (d.voc         || 0) > THRESHOLDS.voc).length },
//         ],
//       });
...
//               <Grid item xs={12} md={dashData.co2Data.values.some(v => v > 0) ? 3 : 6}>
//                 <Box sx={fadeIn(0.7)}>
//                   <TempWidget 
//                     title="Temperature" 
